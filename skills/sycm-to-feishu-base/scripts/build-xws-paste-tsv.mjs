...
const EXPECTED_HEADERS = ['序号', '商品图片', '商品标题', '商品链接', '价格', '月收货人数', '类目', '同款数', '平台', '占位类型', '店铺名', '店铺旺旺', '店铺类型', '地址', '收藏人数', '卖点'];
...
function parseCsv(csv) {
  ...
  if (cell !== '' || row.length > 0) { row.push(cell); if (row.some(value => value !== '')) rows.push(row); }
  return rows;
}

export function buildXwsPasteTsv(csv) {
  const rows = parseCsv(csv.replace(/^\uFEFF/, ''));
  const headers = rows.shift();
  if (!headers || headers.length !== EXPECTED_HEADERS.length || headers.some((value, i) => value !== EXPECTED_HEADERS[i])) {
    throw new Error(`expected exactly 16 columns: ${EXPECTED_HEADERS.join(',')}`);
  }
...
async function main() {
  ...
  if (!input || process.argv.includes('--help')) {
    console.log('Usage: node build-xws-paste-tsv.mjs INPUT.csv > paste.tsv');
    process.exitCode = input ? 0 : 1;
    return;
  }
  process.stdout.write(buildXwsPasteTsv(await readFile(path.resolve(input), 'utf8')));
}
