...
`--shop` **only names the shop inside the alert** — it does not change what gets probed. Probing always hits the default `--proxy` (`19023`, the shared merchant browser), so looping `--shop` over four shops without `--proxy` yields four **byte-identical** results that read like "all four verified" while actually verifying one machine four times. Pass each shop's own proxy (`19041`/`19042`/`19043`/`19044` from `SHOP_BROWSERS`) to probe that shop's window; the receipt's `proxy` field is the tell — if all four receipts are identical, stop, that is a wiring gap rather than four healthy shops. Measured 2026-09-19 05:1x.
