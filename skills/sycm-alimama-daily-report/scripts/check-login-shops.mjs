...
//   2. **一个页面都不碰**：`--check-only` 让 login-merchant 在读完登录态之后就返回，
//      不会去 `/new` 那个淘宝登录页（不带这个开关时它会 —— 那是自动登录的第一步）。
...
// 退出码：0＝全部确认在登录态；2＝有后台明确掉登录；3＝这一层没有结论（读不到/子进程失败）；
 ...
 */
function probeShop(shop, timeoutMs) {
...
async function main(argv) {
  ...
  if (opts.help) {
    console.log('node skills/sycm-alimama-daily-report/scripts/check-login-shops.mjs'
      + ' [--shops a,b] [--json] [--timeout 180000]');
    console.log(`  店名取登记表（runtime/browser-ports.mjs）：${valid.join(' / ')}`);
    return 0;
  }

  const shops = opts.shops ?? valid;
  // 并行：一家店一个浏览器实例、一个代理，互不相干。串行的话五家店要等 5 倍的长。
  const probed = await Promise.all(shops.map(async (shop) => ({ shop, ...(await probeShop(shop, opts.timeoutMs)) })));
  ...
  if (opts.json) {
    console.log(JSON.stringify({ machine: os.hostname(), verdict: judged.verdict, ...judged, rows }, null, 1));
  } else {
    console.log(renderReport({ rows, machine: os.hostname() }));
