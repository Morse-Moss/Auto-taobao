...
```
schtasks /Create /TN "sycm-daily-round" /SC DAILY /ST 11:40 /TR "<见下>"
```

其中 `/TR` 的内容是：先进项目目录，再跑上面那条命令（写成一行）。
注意 `/TR` 里的路径要用反斜杠、整条命令要带引号 —— 计划任务的解析器与 shell 不是一回事，
直接粘一条带引号的 node 命令进去会被拆错。
