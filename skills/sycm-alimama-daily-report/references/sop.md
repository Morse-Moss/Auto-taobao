...
开页与盘点在仓库内（2026-09-19 晚收编，口径与用法见 §13）：

```
node runtime/shop-pages.mjs          # 只读盘点：每个后台缺哪一页、哪一页能「领回」
node runtime/shop-pages.mjs --open   # 可领回的导航回去；确实没有的才真开；同一后台多于一页时只报不动
```

**「缺页」分两种，处置相反（2026-09-20 补，踩过一次）**：工作页被链的第 5 步留在报表预览 URL 上时，
体检会报「缺工作页」—— 但那一页**一页都没丢，只是漂到别的 URL 了**。旧版这里直接新建 ⇒ 同一个主机
变成两页 ⇒ 下一轮 `resetSycmPage` 看到 2 个漂移页、按 fail-closed 停手，整家店一步都不跑。
现在先「领回」（同主机恰好有一页不属于任何期望页面 ⇒ 导航回期望 URL），只有同主机一页都没有时才新建；
同主机多于一页时只报不猜（进退出码 3）。**判据在 `runtime/shop-pages.mjs` 的 `findReclaimCandidates`**。

**安全默认与旧探针相反**：旧探针（仓库外 `D:/Retire/probe-live/86-open-shop-pages.mjs`）是
「不带 `--dry-run` 就真开」，收编后改成「**不带 `--open` 就只盘点**」——
这一步动的是存活浏览器，默认值的失败方向必须是「什么都没发生」。
归位飞书页仍是一个仓库外的专项探针（`90-fix-feishu-page.mjs`）；一次性探针继续放仓库外，
因为仓库内的临时脚本会被端口守卫扫到。
...
| 复核报 `not-hit`，且 `遮挡物` 是个**全屏** div | 那是**平台自己的推广弹窗压住了整页**，不是选择器选错。判据：遮挡物 `position:fixed`、`width/height:100%`、`z-index` 五位数。2026-09-19 实测（盖文淘宝）：「下载报表」`命中自己=0`、`遮挡物={"tag":"DIV","cls":""}`。**它稳定复现**（同一坐标连测两次一模一样），所以**上面那条「等一会儿重跑」不适用** —— 等多久都不会好。**而且弹窗 id 会变**：04:43 那台是同类的 `#wrapper_dlg_982`（`data-owner-id=universalBP_tool_auto_dlg`「优质计划防停投」），09:5x 同一台同一站点已变成 `#wrapper_dlg_925`（`data-owner-id=app`，里面并排着「立即报名」与「关闭」）⇒ **硬编码 id 的探针会报 `present:false` 然后静默 exit 0，让人误以为「没有弹窗」**。改用按「谁盖住视口」定位的探针：`node D:/Retire/probe-live/130-close-overlay.mjs <店铺叫法>`（默认只读，打印候选），确认选点后再 `--apply`。它按「盖住视口 ≥85% 且 z-index 最高」认遮挡层，在层内只取「中心点命中自己」的小元素当候选，并带**危险词黑名单**（报名／开通／购买／支付／确认／提交／领取／升级／续费／立即 —— 命中这些的候选永不被选中，否则盲点会点到「立即报名」这种有对外副作用的按钮），优先选文案含「关闭／close／取消」的；点完**回读**「全屏层是否还剩」+「下载报表中心点是否命中自己」，两条都过才 exit 0。**别去猜按钮坐标**：关闭控件是弹窗里的 16×16 图标按钮，位置随弹窗尺寸变 |
...
- 三个退出码：`0` 全部确认在登录态／`2` 有后台明确掉登录／`3` 这一层没有结论（读不到）／`4` 参数错。
  **`3` 与 `0` 刻意分开**：冷启动后页面还没归位时它读不到，若那时回 0，日志里那一行就与
  「体检真的过了」长得一模一样。
- 告警与「有没有真的去登」绑定（2026-09-23 按用户拍板改，原话「如果自动登录失败就飞书告警，
  但是前提是你要先自动登录」）：
  · **只读档**（不带 `--login`）⇒ 子进程固定拿 `--notify off`，**一个字都不发**，报告里还会
    明写「只读：不开页面、不点任何东西、也不发任何告警」—— 那一轮没有任何登录发生过；
  · **带 `--login`**（真的会开登录页、补手势、提交表单）⇒ 子进程拿 `--notify auto`，
    即「真的试过（`--commit`）且结论仍需要人」时**才**发，**每家店一条**（告警编号含店名与日期，
    同一天同一家店只会被叫一次）。发没发、被没被去重、发失败没有，都透传进报告末尾的 `[告警]` 段。
  · 取值为哪个由纯函数 `notifyModeFor()` 算（`check-login-shops-core.mjs`），用例钉住「没登过就不许叫人」。
- 它只查**一店一实例那五家**，不查商家共用浏览器（19022/19023）—— 理由写在脚本头部。
  ⚠️ 代价（2026-09-23 实测）：商家浏览器自己也有一张「生意参谋工作页」，链的第 0 步要求它恰好一个
  ⇒ **它掉登录会让整轮一步都不跑，而这一层不会报它**。跑链前要单独看它
  （`node runtime/shop-pages.mjs`，只读）。
...
## 13. 定时无人值守（2026-09-19 接：一条命令 + 出错才叫人）

**目标口径**：Windows 任务计划到点敲**一条命令**；「今天能不能写」的判据与「出事叫谁」都在命令里
（三层划分见 `docs/ops/MULTI-SHOP-AND-INTERACTION-DECISION.md` §3）。

```
node skills/sycm-alimama-daily-report/scripts/run-multi-shop-day.mjs --date yesterday --commit --notify
```

三个参数各解决一件必须解决的事：

- **`--date yesterday`**：调度器不必自己算日期。解析用的是落位脚本同一对函数
  （`shanghaiToday` / `shiftIso`），**时钟只取一次**（跨零点不会漂成另一天）。认不出来的取值
  （`today` / `yestoday` / `2026-9-1`）当场报错并列出正确写法 —— 静默落成「今天」会让整整一天的
  数据错位，而那从产物文件名上看不出来。缺 `--date` 仍然抛错（默认不放宽）。
  **定时跑的那一天必须落在「昨日」这一档**：SYCM「询单到付款」里那行「同行同层均值」只有
  预设「1天」（＝昨日）才有（§5）⇒ 定时不需要 `--allow-missing-peer`（那是补历史日才开的降级开关）。
- **`--commit`**：真写。目标日已有「同店同日」的行会硬重复停止（删法见 §9.3）。
- **`--notify`**：**出错才发飞书**，成功一声不响；且只在 `--commit` 那一档发 ——
  排练/只读核对的失败是「你正看着屏幕时的事」，发到飞书只会训练人忽略这个通道。
  想看文案但不想真发：`--notify-print`（用真渲染器打印，**一次投递都不发生**）。

**告警长什么样**：标题自带主体名与统计日；正文说清哪几家收完、哪几家没跑完、停在第几步
（一律中文步名，正文里不出现英文阶段名）、下一步去哪台机器的哪个窗口做什么、以及补跑命令。
按现场分五类，每类有自己的「原因 + 下一步」；撞上「这一天已经写过」的那类明确写「不用处理」
（叫一个不该动手的人去动手，比不提醒更糟）。投递走既有的 `runtime/notify-feishu.mjs`（零参数 CLI），
不另造链路；它「没送达必然非零退出码」，驱动把结论如实打在日志里。

注意：告警**只报错、不代办** —— 它不会替你补页面、也不会自动重试（没实现的行为不许写进提醒）。

**上定时之前先干验证一次**（只读、不写、不投递）：

```
node skills/sycm-alimama-daily-report/scripts/run-multi-shop-day.mjs --date yesterday --notify-print --logs runtime/multi-shop-dry-run-<日期>
```

它会跑一遍第 0 步体检并渲染出告警文案。**触发前必须清掉的前提（驱动不会自己补）：**

| 谁 | 要有什么 | 缺了会怎样 |
| --- | --- | --- |
| 商家浏览器（19022/19023） | 生意参谋工作页 + 飞书底单页，各**恰好一个** | 整轮不跑（第 0 步体检拦住）并发告警 |
| 每台店浏览器（19031~19035） | 生意参谋工作页 + 阿里妈妈报表页，各**恰好一个** | 那家店一步都不跑 |
| 每台店代理（19041~19045） | 进程在（一店一个） | 连不上 ⇒ 那家店体检不过 |
| 登录态 | 各窗口里两个后台都在登录态 | 采集段失败并告警（登录要人做，见 §11） |

工作页**不会自己回来**：新起的浏览器是空的（§1.4），页面被关掉也不会自动重开。
**驱动也不会替你补页**（体检只查不见即报缺，所以缺页＝整轮不跑＋一条告警）。
补页是**独立的一步**，工具在仓库内（`runtime/shop-pages.mjs`）：

```
node runtime/shop-pages.mjs            # 只读盘点：每个后台缺哪一页、哪一页能「领回」（默认不写）
node runtime/shop-pages.mjs --json     # 同样的结论，机器可读
node runtime/shop-pages.mjs --open     # 可领回的导航回去；确实没有的才真开，一律钉住
```

**「缺」分两种，处置相反（2026-09-20）**：链的第 5 步会把工作页留在报表预览 URL 上，于是体检报
「缺工作页」—— 那一页没丢，只是漂了。先**领回**（同主机恰好一页不属于任何期望页面 ⇒ 导航回期望 URL），
同主机一页都没有才**新建**，多于一页只报（退出码 3）。理由与事故链见 §1.4。

**安全默认是反过来的（2026-09-19 收编时改的）**：早先这一步是仓库外探针
（`D:/Retire/probe-live/135-expected-pages-inventory.mjs` / `86-open-shop-pages.mjs`），
那两个**不带 `--dry-run` 就会真开**；收进仓库后改成**不带 `--open` 就只盘点**。
理由是这一步会动存活浏览器，而「忘了加 dry-run」比「忘了加 open」容易得多 ——
默认值的失败方向必须是「什么都没发生」。领回同样只在 `--open` 下发（动作名 `would-reclaim` → `reclaim`）。

计划里的期望页面与驱动**同源**（`buildPagePlan()` 取自 `expectedPagesForShop()` / `expectedPagesForDailyBrowser()`），
所以「盘点说齐了、体检说缺」这种两处口径不一致在这里不会发生。
退出码：`0`＝每个期望页面恰好一个；`2`＝有缺口或代理连不上；`3`＝有需人决定项（同一后台多于一页，或漂移页认不出该领回哪个）。

补页口径四条（都在 §1.4）：**缺页先领回再考虑新建**、**只开真缺的**、
**同一后台多于一页就只报不动**（关哪一个是人的决定）、**新建一律 `pinned=1`** ——
不带 `pinned` 的页闲置 15 分钟就被代理回收，表现为「今晚补好、明早没了」且不报错。
补完再跑一次只读盘点复核，然后跑上面的干验证命令。

**2026-09-19 晚实测（只读盘点，未补）**：商家浏览器缺飞书底单页；盖文淘宝／盖文天猫各缺阿里妈妈报表页；
里可林淘宝／网林天猫／科塔淘宝**两页全缺**（这三家的浏览器被回收过）。所以「挂定时」之前这一步是硬前提。

**当前宿主：WorkBuddy 平台的定时任务**（2026-09-21 起；触发层只负责叫醒，判据在命令里）。
...
- **换宿主铁律：先撤旧的、再挂新的。** 两个触发器并存会在同一时刻各跑一次 ⇒ 同目标日重复写飞书。
  2026-09-20 挂过 Windows 任务 `sycm-daily-round`，已于 09-21 撤销（撤销前它一天都没真跑过）。

备选路径（Windows 计划任务）。入口命令由仓库自己拼，**不要手抄** —— 计划任务的解析器与 shell 不是一回事：

```
node scripts/schedule-install.mjs            # 只打印（默认）：任务名、触发器时刻、将执行的那条命令
node scripts/schedule-install.mjs --install  # 真挂（/F 覆盖同名任务）
node scripts/schedule-install.mjs --query    # 只读：挂了没有、下次什么时候跑
```

**已知的工具缺口**：上面三条都走 `schtasks.exe`，而它在本机程序黑名单里 ⇒ 一律 exit 3
（脚本会如实报「读不出来」，不是「没挂」）。要恢复这条路径，三条任选：
① 在「安全中心 → 命令安全 → 程序黑名单」里移出 `schtasks.exe`，再跑 `--install`；
② 用任务计划程序界面手工挂（`taskschd.msc`，程序＝node 完整路径，参数＝脚本路径）；
③ 直接用 PowerShell `Register-ScheduledTask` 注册等价任务（参数照
`evidence/schedule-register-2026-09-20/01-schedule-print.txt`）。注意黑名单横幅写着「不要做等价替代」——
③ 属绕过，做之前要用户点头。
`--install --time` 早于 `11:20` 会被脚本当场拒掉 —— 那是 upsert 前的事实判据，不是忠告。

**触发时刻为什么是 11:40 而不是清早**：同一目标日、不同时刻导出的阿里妈妈 CSV **不一样**
（§6.5 实测：08:04 那份与 11:22 / 12:36 / 12:5x 三份逐字节不同，而 11:22 之后那三份完全相同）。
设计上本该由一条**自动判据**（再导一份、与手上那份逐字节相同才写）替掉这条「别太早跑」的咒语，
**那条判据目前只写在文档里、链里没有实现** ⇒ 在它实现之前，触发时刻本身就是唯一的安全边界，
不要往 11:20 之前放。

**还没接上的（别把「脚本化」说成「自动化」）**：排期配置 `runtime/round-schedule.json` 与
`sop-runtime` 的排期/幂等体系**没有接这条链**（日报能力还没注册进能力表，所以「换宿主只改配置」
这一层还不成立，现在挂的就是直接跑这一条命令）；体检层只实现 L1（§12.8）。
**进程保活**：`scripts/start-all.mjs` 按登记表起齐声明实例、幂等、已就位的不碰，并由
`scripts/run-daily-job.mjs` 在跑链之前先调一次（所以触发器不需要人先手工把浏览器开好）。
宿主换成会话之后这个问题**变小了**：`run-daily-job.mjs` 是**前台阻塞**跑完两步才退出，
所以那些 `detached` 实例只需活到「同一条命令跑完」，不需要活过宿主的进程树 ——
命令结束时链条早已跑完，随后整棵树被回收是正常的。
**但「活到命令跑完」这一次也没有实测**（本项目在 agent 会话里实测过 `detached:true`
**没有**让子进程脱离宿主进程树，那是沙箱行为）⇒ 2026-09-21 11:40 那一次真跑就是它的第一次验证。
所以这一档现在的诚实说法仍是：「**一条命令跑完全链、起实例也在命令里**」，不是「没人管也能跑」。
真无人值守还差三件：进程保活**已验证归属**（差一次实测）、缺页自愈、体检层往上做（L2/L3）。
