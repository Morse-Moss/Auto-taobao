...
**原因未查明**（不排除 Edge 对不活跃标签页的回收；本次期间机器恰好跑了两轮各 20 分钟的全量回归，
内存压力大）。**不去编一个解释**。但结论一样：**不要假设「挂一次就永久有效」**。

三条应对，按确定性排序：

1. **§1.3 的位置本来就是对的**：它是「起浏览器之后」的一步 ⇒ 每次起浏览器都挂一次，就不受影响。
   判别方法很直接：只读报告里 `labelTabs: 0` 就是缺。
2. **发告警之前重挂一次更稳**（`--commit` 是幂等的，已有就导航、不会多出页签）——
   因为告警发出去到人走到机器前还有时间差，中间正好是它可能消失的窗口。**这一条尚未接线**。
3. 不要为了它去动浏览器/操作系统的配置（那是用户的机器，且本项目纪律是未经许可不动进程/配置）。
