...
```powershell
node runtime/shop-window-label.mjs                                        # 只读报告：每家店窗口现在有哪些页签
node runtime/shop-window-label.mjs --commit                               # 挂上/更新标签页（幂等）
node runtime/shop-window-label.mjs --commit --only 盖文淘宝
node runtime/shop-window-label.mjs --commit --state "需要登录" --ok 0      # 量到状态时一并写上去
```
...
2026-09-18 晚实测的四台窗口（`node runtime/shop-window-label.mjs` 的真实输出）：

| 窗口 | 标签页 | 页签构成 | 当时的登录态 |
|---|---|---|---|
| 19031 里可林淘宝 | **0 个（缺）** | 工作页 ×2、千牛 ×2、空白 ×1 | 两个后台都已登录 |
| 19032 网林天猫 | 1 个，写的是会员名 `网林家居旗舰店:阿彦` | 工作页 ×2、标签 ×1、千牛 ×2、空白 ×1 | 两个后台都已登录 |
| 19033 盖文淘宝 | 1 个，写的是会员名 `随心品质定制:阿彦` | 工作页 ×2、标签 ×1、千牛 ×1、空白 ×1、登录页 ×1 | **未登录**（sycm 被踢回 `custom/login.htm`、alimama 停在 `#!/login/index`） |
| 19034 科塔淘宝 | **0 个（缺）** | 工作页 ×2、千牛 ×3、空白 ×1 | **阿里妈妈未登录**（`#!/login/index`） |
...
突变验证 **5/5** 被捕获（探针 `D:/Retire/probe-live/33-mutate-window-label.mjs`，仓库外），
含 N1「标题不带店名」、N2「无条件写 state」、N3「工作页标成不必需」、N4「堆多个照样导航」、
N5「不再写 `document.title`」。
