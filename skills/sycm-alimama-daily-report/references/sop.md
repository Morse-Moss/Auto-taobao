...
入口命令由仓库自己拼，**不要手抄** —— 计划任务的解析器与 shell 不是一回事，手抄的引号会被拆错：
...
`/TR` 里只有**一个**入口 `scripts/run-daily-job.mjs`，它自己按 `runtime/daily-job-plan.mjs`
的顺序跑两步（① `scripts/start-all.mjs` 把声明实例起齐；② 上面那条全链命令）。
两步的默认值都在那份计划里被断言：日期只用 `--date yesterday`（不写死日期）、
**不加 `--allow-missing-peer`**（那是补历史日的降级开关）、**告警默认 `--notify-print`**
（文案落日志、一次投递都不发生；要真发飞书必须显式 `--notify`）。

**2026-09-19 实测的硬约束：本机把 `schtasks.exe` 列进了安全策略的程序黑名单** ⇒
`--install` 与 `--query` 在我这边**都执行不了**（脚本会如实报「读不出来」，不是「没挂」）。
所以注册得由人来做，三条路径任选：
① 在「安全中心 → 命令安全 → 程序黑名单」里移出 `schtasks.exe`，再跑 `--install`；
② 用任务计划程序界面手工挂（`taskschd.msc`，程序＝node 完整路径，参数＝脚本路径）；
③ 用平台定时任务每天跑同一条命令（兜底定位，理由见 `docs/ops/MULTI-SHOP-AND-INTERACTION-DECISION.md` §3）。
...
**进程保活（2026-09-19 晚的新状态）**：以前是「浏览器进程得由别的东西保活」，现在
`scripts/start-all.mjs` 成了那个「别的东西」—— 它按登记表起齐声明实例、幂等、已就位的不碰，
并且由 `scripts/run-daily-job.mjs` 在跑链之前先调一次（所以计划任务不再需要人先手工把浏览器开好）。
**但有一件事没实测**：由任务计划拉起的那些 `detached` 进程，能否活过任务本身这个进程树，
**没有验证过**（本项目在 agent 会话里实测过 `detached:true` 并**没有**让子进程脱离宿主的进程树，
那是沙箱行为，任务计划里未必一样）。所以这一档现在的诚实说法仍是：
「**一条命令跑完全链、起实例也在命令里**」，不是「没人管也能跑」。真无人值守还差三件：
进程保活**已验证归属**（差一次实测）、缺页自愈、体检层往上做（L2/L3）。
