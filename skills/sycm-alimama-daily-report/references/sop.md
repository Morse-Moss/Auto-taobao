...
3. 下载 CSV 营销场景报表。⚠️ 操作按钮（`下载`）在**文件名所在 `tr` 的下一个 `tr`** 里，按「同一行」去找会
   「任务明明已生成成功、脚本却一直等」且不报错 —— 定位手法与判据见 §4.2，先读那一节再动手。
...
### 4.2 「任务已生成成功，但脚本一直等不到」的根因（2026-09-17 实亏一次，务必先读）

2026-09-17 演示时卡在这里：阿里妈妈那边**任务列表里早就写着「生成成功」**，脚本却空转了 3 分钟。根因不是任务没生成，是**下载按钮的位置判断错了**。

- **任务行的结构与直觉不一样**：文件名（`营销场景报表_20260917_112258`）在一个 `<tr>` 里，
  而操作区（`下载` / `导入超级表格` / `AI分析` / `删除`）在**紧邻的下一个 `<tr>`** 里。
  按「操作按钮在文件名同一行」去找，永远找不到 —— 而且它不报错，只是一直等。
- 正确做法：定位到文件名那个叶子元素 → `closest('tr')` → 用 `nextElementSibling` **往后走最多 4 个兄弟**
  找那个文本恰为 `下载` 的叶子元素（实测在 i=1 命中）。点击前再用 `elementFromPoint`
  按矩形中心复核一次命中，避免点到浮层或落空。

第二个要改的是**「下载成功没有」的判据**：

- 不要用页面上的按钮状态或固定 `sleep` 判。判据取**文件系统**：记录点击前 Downloads 里的 zip 列表，
  点击后每 3s 扫一次，出现**新**的 `营销场景报表_*.zip` 才算成功（实测 809 字节、内部日期＝目标日）。
- 任务本身生成得很快 —— 实测十几秒内状态就已变成「生成成功」，所以「等不到」几乎总是**定位逻辑**的问题，
  不是任务没生成。先去看任务行的状态文本，再去看按钮在哪一行，别加大轮询时长。

顺带更正 §4 第 3 步那句「最后一次下载按钮需要 CDP 真实鼠标事件」：2026-09-17 实测这个任务行里的
`下载` 按钮用 JS 层的 `POST /click`（即 `el.click()`）**也能**触发下载。真实鼠标事件（`/clickPoint`）
是备用手段 —— 哪天 JS 点击不触发下载了再退回它，不必一上来就用。
...
| 第 8–9 步 | 生意参谋（询单到付款） | 回填读的是这一页的数；它不改 URL，要显式切 |
