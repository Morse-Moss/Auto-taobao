...
```
node runtime/shop-pages.mjs          # 只读盘点：每个后台缺哪一页
node runtime/shop-pages.mjs --open   # 真开（只开**缺的**那些，已有的不动，同一后台多于一页时只报不动）
```
...
```
node runtime/shop-pages.mjs            # 只读盘点：每个后台缺哪一页（默认不写，不带 --open 就只盘点）
