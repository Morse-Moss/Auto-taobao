...
每台店浏览器挂一个标签页：`document.title` = 店名（**任务栏/窗口列表里显示的就是这一行**），
页面正中用大字再写一遍店名。

```powershell
node runtime/shop-window-label.mjs                                        # 只读报告：每家店窗口现在有哪些页签、哪家停在登录页
...
**上一版标签页的缺陷**：它写的是阿里妈妈页头的**会员名**（`随心品质定制:阿彦`），而运营在飞书里看到的是
**运营叫法**（`盖文淘宝`）—— 两边对不上，等于白挂。口径定死：**标签页与窗口标题一律用运营叫法**
（`SHOP_BROWSERS` 的键）；会员名/页头店名要展示只能作为附加小字。这是「同一家店有四个名字」那个坑
在**界面侧**的又一次发作（前两次分别在采集身份核对与证据目录键上）。
