...
## 17:48 复核（用户第二次问「人工看没什么不一样」之后做的同一字段并排）

见 `FRESH-RECHECK-2026-09-21-1748.md`。三样新产物，全部只发 GET / 只读 DOM：

- `sycm-insight-5shops.*` —— 五家店 `all_modules` 台账里「服务洞察」那一项的并排原文（同一时刻一起读）。
- `sycm-plainpath-5shops.*` —— 同一条**不带 `/new`** 的 GET 发给五家各自的当前页签，看服务端应答。
- `keta-home-text.*` —— 科塔被弹走之后落下的那张页面的**可见文字全文**（509 字，逐字列出后无任何「无权限」字样）。

结论没有变化，但多了一条时间线：科塔最后一次成功取数是 2026-09-20 16:15（本地，源表 sha256 在案），
首次失败是 2026-09-21 14:53 —— 失效窗口约 09-20 17:37 至 09-21 14:53。
