// 把 2026-09-24 这一天「五家店」的数据清掉（第二次：为「删掉后重跑一整轮、验证数据与释放」做准备）。
//
// 与 09:24 那次（evidence/daily-report-2026-09-24-clear/）是同一份逻辑，只有 OUT_DIR 不同 ——
// 那次是清掉 09-24 补跑的第一版，这次是清掉 11:53 那轮补进来的正式版本（用户 2026-09-25 18:57 指示）。
//
// 为什么只清两处（见下方每条判据的出处）：
...
// 回执写到一个**新的**目录：09:24 那次清理已经把 pre/post 写进 …-clear/，
// 重跑本脚本若复用同一个 OUT_DIR 会覆盖掉那次的证据（脚本头部第 6 条自己注明过这件事）。
const OUT_DIR = 'D:/Retire/sycm-automation/evidence/daily-report-2026-09-24-clear-rerun';
