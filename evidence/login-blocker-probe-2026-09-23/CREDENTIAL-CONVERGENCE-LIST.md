# 需要收敛的淘宝凭据清单（2026-09-23 晚 · **已按本清单执行，见文末**）

> **2026-09-23 晚更新：这份清单已经执行完毕。**
> 用户当轮原话「商家浏览器这个，是哪个商家就留哪一个 / 授权给你，确保明天的定时自动化顺利」。
> 实际删了 **4 条**（两台各 2 条，比下面「必删 2 条」多了同属外来的 2 条 `havanalogin` 死重），
> 逐条、备份路径、回读自证、同步复核、以及顺带发现的两个假红缺陷，
> 全部记在 **`CREDENTIAL-CONVERGENCE-DONE-2026-09-23.md`**。本文件保留为「当时怎么判断的」那一步。
