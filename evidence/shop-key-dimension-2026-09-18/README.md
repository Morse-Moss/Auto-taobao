...
在它之外另跑了一次 `run-test-suite.mjs unit`（技能 + 运行时全量离线）作为回归扫，原文在 `05-unit-all.txt`：

- 技能那一半：**652 / 652，0 失败**；运行时那一半：**596 / 596，0 失败**（两段 TAP 各自 `# fail 0`）。
- 跑的时候工作区已经是提交 `1a095c1` 的内容（本批改动全部在内），也就是**这一批的绿灯**。
- 基线往后看这里：技能全量从 609 上移到 **652**（本批 +6，其余增量来自今天更早的提交）、
  运行时从约 593 到 **596**。数字变化不等于回归，但**报绿灯时必须连版本一起说**。
