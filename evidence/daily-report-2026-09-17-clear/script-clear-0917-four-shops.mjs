// 把 2026-09-17 这一天「四家店」的数据清掉，为「删除再重写」做准备。
//
// 为什么只清四家、而且只清两处（见下方每条判据的出处）：
//   1) 底单里 09-17 有 5 行，第 5 行是「盖文旗舰店」（=盖文天猫）。那条链**现在没有能力重放它**
//      —— 它需要一台登录成盖文旗舰店的浏览器，而本机五个浏览器里没有（19022/19023 实测是里可林家居）。
//      删了就补不回来 ⇒ 默认拒绝删它。
...
const OUT_DIR = 'D:/Retire/sycm-automation/evidence/daily-report-2026-09-17-clear';

// 本轮要重写的四家（运营叫法）。**必须与登记表一致**，不一致就别跑。
const REWRITE_KEYS = ['里可林淘宝', '网林天猫', '盖文淘宝', '科塔淘宝'];
// 09-17 那一行属于盖文天猫 —— 保留它（删了没有通路补回来）。
const KEEP_SOURCE_SHOP = '盖文旗舰店';
...
const toDelete = sourceRows.filter((row) => row.shop !== KEEP_SOURCE_SHOP);
const kept = sourceRows.filter((row) => row.shop === KEEP_SOURCE_SHOP);
console.log(`  ⇒ 待删 ${toDelete.length} 行：${toDelete.map((r) => r.shop).join(' / ')}`);
console.log(`  ⇒ 保留 ${kept.length} 行：${kept.map((r) => r.shop).join(' / ') || '（没有）'}`);

// 判据 A：保留的必须恰好是盖文旗舰店那一行（否则「哪一行补不回来」这件事就不可知了）。
if (kept.length !== 1 || kept[0].shop !== KEEP_SOURCE_SHOP) {
  fail(`底单里 ${DAY} 的「${KEEP_SOURCE_SHOP}」不是恰好 1 行（实际 ${kept.length} 行）——`
    + '它是本链现在没有能力重放的那一行，删错就补不回来。');
}
// 判据 B：待删的四家必须逐字等于本轮要重写的那四家（多一家少一家都停）。
const deleteShops = toDelete.map((r) => r.shop).sort();
if (deleteShops.length !== REWRITE_KEYS.length) {
  fail(`待删行数 ${deleteShops.length} ≠ 本轮要重写的店铺数 ${REWRITE_KEYS.length}。`);
}
...
const snapshot = {
  ...
  decision: { deleteSource: toDelete.map((r) => r.recordId), keepSource: kept.map((r) => r.recordId),
...
console.log(`\n删后回读：底单 ${report.source.before} → ${report.source.after}（${DAY} 剩 ${report.source.onDayAfter} 行：${report.source.remainingShops.join(' / ')}）`);
