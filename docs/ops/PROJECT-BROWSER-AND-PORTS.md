...
`runtime/browser-ports.test.mjs` 的五条测试盯着（路线表完整性、站点账号边界、
路线与 `skills/` 双向一致、每个浏览器承载哪些路线、别的项目代理的欠债清单）。
...
**这张表说的是「应该去哪」，不等于「已经接线」**：`sycm-export-search-rank`、`sycm-to-feishu-base`
的代码默认值今天仍指向别的项目的 `3456`（欠债清单见 §1.4 与 §3，共 31 处），
所以乙这条链目前只有日报那一半是真的在 `19022`/`19023` 上跑。迁不迁、什么时候迁，等拍板。

## 1.4 还没定的一件事：灰豚（关键词链的一半）借用了别的项目的代理

**现状**：`huitun-to-feishu-keyword-heat` 的采集走 `http://127.0.0.1:3456` —— 那是**另一个项目**
的共享 CDP 代理，挂在**使用者日常 Edge** 上，`/health` 报 `browser.id=edge`。

**为什么这是欠债**：这条链能不能跑，取决于
「别的项目的代理此刻活着」＋「那个浏览器里恰好登着灰豚」。别人关掉代理，断的不是他们的流程，
是我们的。这与 §2 里「进程必须由本会话启动」是同一个问题的两种形态。

**为什么不能靠「归到甲还是乙」直接决定**：灰豚是独立第三方平台（`xhs.huitun.com`），
用的是它自己的账号，与淘宝买/卖身份都无关，所以放哪边都不冲突。
真正的代价是**一次性登录**：无论最后选哪一边，都要在那个 profile 里重新登一次灰豚。

**候选**（等拍板，代码里已用 `browserPending` 显式标着，不是「没人记得」）：

1. 归 **乙（商家号浏览器 `19023`）** —— 关键词这条路线两半（搜索排行 ＋ 灰豚）就都在乙里，
   跑关键词不用开两个浏览器；代价是乙里要重登一次灰豚。
2. 归 **甲（买家号浏览器 `3457`）** —— 甲本来就是「对外采集」浏览器（小旺神也是第三方插件），
   乙保持「纯后台」的干净面；代价同样是重登，且关键词路线要开两个浏览器。
3. **保持借用 `3456`** —— 一分钱不花，但故障源在别人手里。

顺带一条同源欠债：**飞书网页登录态现在也挂在 `3456`**（`sycm-to-feishu-base` 的粘贴路径、
`runtime/feishu-ui-*.mjs` 等 20 个人工维护探针都默认指向它）。若要让乙成为完整的「商家链」，
这条迟早要一起搬到 `19023`。目前所有指向 `3456` 的文件都在 `runtime/browser-ports.mjs` 的
`FOREIGN_PROXY_DEFAULT_FILES` 里逐个点名，并有测试盯着清单与现实一致 —— 修一处删一行。
...
- **3456**：别的项目的 CDP 代理，挂在**用户的日常 Edge** 上（`browser.id=edge`）。
  ...
  它的另一个身份是**本项目自己的历史欠债**：2026-09-16 实测仍有 31 个文件把代理默认值指向它
  （11 个生产链路文件 ＋ 20 个 `runtime/` 人工维护探针），逐个点名在
  `runtime/browser-ports.mjs` 的 `FOREIGN_PROXY_DEFAULT_FILES`，并有测试盯着清单与现实一致。
  已经改掉的：`xws-export-market-analysis` 与 `xws-sku-collection` 的 SKILL.md（它们原来写着
  「用共享 Proxy `3456`、要求 `browser.id=edge`」——那是竞品链还在借别人代理时期的话，
  和现行竞品链（`3457` ＋ `edge-isolated`）是矛盾的，会把人直接带到错的浏览器上）。
