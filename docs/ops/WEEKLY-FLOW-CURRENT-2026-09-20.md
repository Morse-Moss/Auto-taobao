...
背景：多份文档对同一批字段记了互相矛盾的数字（见 §八），所以这轮**不采信任一份文档**，直接写只读探针发 GET 实测。
...
| 09-13~09-19 | 39 | 1417 | 100% | **Text 100%** | **Text 100%** | **100%** | Text 98.4% | **0%** |

⇒ **08-30 与 09-06 两期才是真正什么都没写进去的**（除公式列外全 0）；09-13~09-19 这期 12 个关键字段里 10 个 100%、1 个 98.4%、只有 `商品ID` 是 0%。
⇒ 「竞品链跑不通」这个说法与本表不符：采集→导入→属性写回→分类→发布这一段**本期通**，卡住的是末端（见 7.3）。

### 7.2 主表 / SKU 明细 / SKU 周表（实测类型与有值率）

- 竞品主表 `tblkYcczxBnW4v5G`（36 字段 / 2004 行）：`尺寸`、`适用空间` **仍是 Lookup raw_type=19，只有 15/2004（0.7%）**；`数据状态`、`待补数据项` **仍是 Formula raw_type=20**；`搜索关键词` 100%；`商品ID` 1461/2004（72.9%）。
