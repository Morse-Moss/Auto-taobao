...
| **M3** | P0-1 配置外置 ＋ P0-2 一条命令 | 「迁移」这件事从此不再需要工程师 | M1 | **P0-1 已完成**（`config/customer.json` ＋ `runtime/customer-config.mjs`）；**P0-2 部分落地**（2026-09-18 晚：`run-multi-shop-day.mjs` 把「落位→导出→取件→干跑/核对→回填→回读」固化成一条命令并真跑通，四家店串行、只读复验 —— 见下方偏差说明） |
...
1. **体检段未接线** —— 那条命令的规格里第一步是体检，而 `runtime/xws-platform-health-preflight.mjs`
   只有 L1（环境层）真实实现，L0/L2/L3 如实标 `NOT_IMPLEMENTED`，也还没接进驱动。
