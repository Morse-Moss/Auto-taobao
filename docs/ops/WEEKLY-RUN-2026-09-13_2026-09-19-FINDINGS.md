...
问题库_2026-09-13_2026-09-19  tblQeUSb8WQWN2dF  50 行 / 23 字段   （base 表数 11 → 12）
  覆盖：来源记录唯一键 50  分类标签 50  原始内容 50  周期开始/结束日期 50/50  痛点描述 13
  是否痛点分布：是 4 / 否 46
状态机          status DONE  nextAction DONE  summariesPublished true  operatorRecords 2073
```

两点口径澄清（运营看表时会遇到，先写清楚）：

- **`痛点描述` 只有 13 行有值，不是漏写**。它是**标签级**字段，来自运营的《高价值竞品浴缸评价分类统计》xlsx（按 `分类标签` 取 `痛点描述/典型问题/典型用户原话`），与**记录级**的 `是否痛点` 不同层；不在运营表里的标签才回落到 `humanReviewReason` 或 `痛点判定依据`。所以「否 + 有痛点描述」不矛盾：该标签本就是已知痛点类别，只是这条记录没提到。
- 50 行 = 27 条源记录展开出的「源 × 话题」二元组，其中判为痛点的 4 条。
