...
- Route ownership is a table, not a habit — `ROUTES` in `runtime/browser-ports.mjs` is authoritative and `docs/ops/PROJECT-BROWSER-AND-PORTS.md` §1.3 is its readable digest. Buyer browser (`9222`/`3457`) carries the competitor chain (小旺神 market analysis, SKU, FAQ, question library) on `s.taobao.com` / `item.taobao.com` / `detail.tmall.com`. Merchant browser (`19022`/`19023`) carries keyword search-rank (生意参谋 搜索排行), the daily report (生意参谋 + 万相台/阿里妈妈 + Feishu), the weekly Feishu paste, and the reserved 千牛/卖家工作台 slot (`myseller.taobao.com` / `qianniu.taobao.com`). Assign a chain by the account its target site requires, never by which browser happens to be open. A seller-version account cannot use 小旺神 at all, so a merchant login on the buyer profile silently degrades collection rather than failing; the 灰豚 keyword-heat route is still undecided and is explicitly marked `browserPending` in that file — do not quietly pick a browser for it.
