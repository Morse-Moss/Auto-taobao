...
test('keeps the legacy 1000w helper diagnostic-only', () => {
...
test('normalizes exact and no-result Huitun evidence', () => {
  assert.deepEqual(normalizeResults(resultDocument).map((item) => ({ keyword: item.keyword, views: item.views, heat: item.contentHeat })), [
    { keyword: '家用浴缸', views: 1_094_000, heat: undefined },
    { keyword: '新型泡澡浴缸', views: 0, heat: undefined },
  ]);
...
test('builds topic-view updates only for the complete live A candidate queue', () => {
  ...
  assert.deepEqual(plan.updates, [
    { record_id: 'rec1', fields: { 灰豚话题浏览量: 1_094_000 } },
    { record_id: 'rec2', fields: { 灰豚话题浏览量: 0 } },
  ]);
...
test('mutation guard accepts only the exact topic-view plan', () => {
...
test('verification allows formula settlement but rejects unrelated changes', () => {
  ...
  const after = [
    record('rec1', '家用浴缸', 'B-持续观察', { 灰豚话题浏览量: 1_094_000 }),
    record('rec2', '新型泡澡浴缸', 'B-持续观察', { 灰豚话题浏览量: 0 }),
  ];
  ...
  const changedContentHeat = structuredClone(after);
  changedContentHeat[0].fields.内容热度 = '高';
  assert.throws(() => verifyHuitunBackfill({ before, after: changedContentHeat, plan }), /unauthorized/i);
});
